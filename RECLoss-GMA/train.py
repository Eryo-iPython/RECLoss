...
def summery(file, count, data):
    with open(file, 'a+') as f:
        f.write(f'{count:06d} {data}')
        f.write('\n')
        f.close()
...
def main(args):
    ...
    if torch.cuda.is_available():
        model = nn.DataParallel(RAFTGMA(args), device_ids=args.device_ids)
    ...
    scheduler = optim.lr_scheduler.OneCycleLR(optimizer, args.lr, args.steps + 100,
                                             pct_start=0.0, cycle_momentum=False, anneal_strategy='linear')
    print('Data ready')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ckpt', default='50points_final_sintel.pth')
    ...
    parser.add_argument('--point_nums', help='how many points to compute fmatrix: default**2', default=10)
