...
def submi_kitti_vis(model, iters=24, outpath=r'kitti_submission'):
    ...
    # img_dir = sorted(glob.glob(os.path.join(root, '*.png')))
    # img1_dir = img_dir[::2]
    # img2_dir = img_dir[1::2]
    # img1 = [np.array(read_gen(img)) for img in img1_dir]
    # img2 = [np.array(read_gen(img)) for img in img2_dir]
    #
    # img1 = [torch.from_numpy(img).permute(2, 0, 1).float() for img in img1]
    # img1 = [img[None].cuda() for img in img1]
    #
    # img2 = [torch.from_numpy(img).permute(2, 0, 1).float() for img in img2]
    # img2 = [img[None].cuda() for img in img2]
...
parser.add_argument('--mixed_precision', help='use mixed precision', default=True)
...
if __name__ == '__main__':
    model  = torch.nn.DataParallel(RAFTGMA(args), device_ids=[0, 1])
    model.load_state_dict(torch.load(r'models/final_kitti.pth'))

    model.cuda()
    model.eval()
    """
    Validation KITTI: 0.569184, 1.223577
    {'kitti-epe': 0.5691843326576054, 'kitti-f1': 1.2235773727297783}
    """
    # submi_sintel_vis(model.module, warm_start=False)
    # submi_kitti_vis(model.module)
    with torch.no_grad():
        # print(validate_sintel(model.module, iters=12))
        print(validate_kitti(model.module, iters=12))
